...
exports.departmentTechnicienReport=(req,res) => {
    code=req.params.code
    var name=null 
    Department.findByPk(code).then(dep => {
        name=dep.dataValues.Name
    ...
    Technicien.findAll({where:{DepartmentCode:code}}).then(tech => {
        if(tech){
            ...
            res.render('reportDepartmentTechnicien',{layout:'departmentReportLayout',pageTitle:'Chef Service',
                                                    code:code,engineers:en,hasEngineers:en.length>0,
                                                CE:true,name:name})
        }
        else{
            res.render('error',{layout:false,pageTitle:'Error',message:'Sorry!! Couldn\'t Find  Any technicien For This Department',href:'/report/department/chefserice/'+code})            
        }
    }).catch(err => {
        if(err)
        res.render('error',{layout:false,pageTitle:'Error',message:'Sorry!!  Error happend while getting Engineers For This Department',href:'/report/department/chefserice/'+code})
    })
